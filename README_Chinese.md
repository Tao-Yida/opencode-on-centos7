...
3. **脚本位置说明**：主脚本 `opencode_with_custom_glibc.sh` 位于项目根目录中以供直接使用。虽然项目仓库中包含一个 `scripts`子目录，但实际运行的脚本放置在根目录中以避免混淆。使用本项目时，请确保使用来自根目录的脚本。
...
**问题原因**：
当 OpenCode 使用自定义 glibc 2.28 时，`libpthread` 需要 `libgcc_s.so.1` 来支持 `pthread_cancel` 功能。
特别是在执行文件搜索、并行处理等复杂任务时，如果找不到该库会导致进程崩溃。

**解决方案**：
启动脚本 `opencode_with_custom_glibc.sh` 已自动处理此问题：
- 设置 `LD_LIBRARY_PATH` 包含 GCC 9.5.0 的 lib64 路径
- 确保只添加 GCC 库路径，不添加自定义 glibc 路径，避免 bash 子进程崩溃
- 这样 OpenCode 可以找到 `libgcc_s.so.1`，但系统命令仍然使用系统 glibc

**验证方法**：
```bash
# 检查 libgcc_s.so.1 是否存在
