...
- 已安装基础开发工具（如未安装，需要 root 权限执行 `yum groupinstall "Development Tools"`）
- 网络连接（用于下载源码）
...
在开始编译 GCC 之前，需要确保系统已安装必要的开发工具和库：

```bash
# 如果尚未安装开发工具组（需要 root 权限）
...
**注意**：如果无法使用 sudo，可以尝试从源码编译这些依赖库，但过程会更复杂。
